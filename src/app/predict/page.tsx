  ...
  useEffect(() => {
    fetch(`${API}/flights/raw`)
      ...
      .then(d => {
        ...
        if (flightParam) {
          const found = list.find((f: FlightRaw) => f.flight?.iata === flightParam)
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '2rem', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              {[
                ...
                <div key={i} style={{ background: 'rgba(245,166,35,0.04)', padding: '0.6rem', borderLeft: '2px solid var(--amber-dim)' }}>
                  ...
                  <div className="mono" style={{ fontSize: '0.9rem', color: 'var(--text)', marginTop: '0.2rem' }}>{item.val || '—'}</div>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Résultat */}
        <div className="panel" style={{ padding: '1.5rem', borderRadius: '4px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {result === null ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <div className="mono" style={{ color: 'var(--muted)', fontSize: '0.8rem' }}>EN ATTENTE DE PRÉDICTION</div>
            </div>
          ) : (
            <>
              {/* Jauge circulaire */}
              <div style={{ position: 'relative', width: '180px', height: '180px', marginBottom: '1.5rem' }}>
                <svg width="180" height="180" style={{ transform: 'rotate(-90deg)' }}>
                  ...
                  <circle
                    ...
                    stroke={riskColor}
                    strokeWidth="12"
                    strokeDasharray={`${Math.min(result / 120 * 471, 471)} 471`}
                    style={{ transition: 'stroke-dasharray 1s ease' }}
                  />
                </svg>
                <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  <div className="display" style={{ fontSize: '2.8rem', color: riskColor, lineHeight: 1 }}>
                    {result.toFixed(0)}
                  </div>
                  <div className="mono" style={{ fontSize: '0.65rem', color: 'var(--muted)' }}>MINUTES</div>
                </div>
              </div>

              <div className="display" style={{ fontSize: '1.8rem', color: riskColor, letterSpacing: '3px', marginBottom: '1rem' }}>
                {riskLabel}
              </div>
